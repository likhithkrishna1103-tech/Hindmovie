...
function debug(elf_) {
    let result = "";
    const elf = elf_.elf ? elf_.elf : elf_;
    if (elf) {
        ...
        for (const section of elf.sections) {
            ...
            if (sections_1.isSymbolSection(section)) {
                if (elf.bits == 32) {
            ...
            else if (sections_1.isStringSection(section)) {
                for (const string in section.strings) {
                    result += `  #${string} - ${section.strings[string]}\n`;
                }
            }
            else if (sections_1.isRelocationSection(section)) {
