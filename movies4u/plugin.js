(function () {
  ...
  if (!safeFetch) {
    try {
      safeFetch = new Function(
        "return (this.constructor.constructor('return fetch')())",
      )();
    } catch (_) {}
  }
  ...
  const Analytics = {
    ...
    logEvent(eventName, parameters) {
      console.log(
        "[Analytics] Event: " + eventName + " | clientId: " + this.clientId,
      );
    ...
    async flushQueue() {
      ...
      try {
        ...
        console.log("[Analytics] Send skipped");
      }
    },
  };
  Analytics.init();
  ...
  function request(url, options) {
    ...
    if (
      ...
      return Promise.resolve(http_post(url, body, headers)).then(
        function (res) {
          return {
            status: res && typeof res.status !== "undefined" ? res.status : 200,
            body: res && typeof res.body !== "undefined" ? res.body : "",
  ...
  function looksLikeMediaListing(html) {
    ...
    return (
      /<article\b[^>]*class=["'][^"']*(?:entry-card|post|hentry)[^"']*["']/i.test(
        source,
  ...
  function validateMainUrl(candidate) {
    ...
    if (!/^https?:\/\//i.test(url) || isPlaceholderBaseUrl(url))
      return Promise.resolve("");

    return getText(url + "/", pageHeaders(url), true)
      .then(function (html) {
  ...
  function resolveDynamicJsHrefs(html) {
    var mappings = {};
    var elementHrefRegex =
      /document\.getElementById\(\s*["']([^"']+)["']\s*\)\.href\s*=\s*([a-zA-Z0-9_$]+)/g;
    var match;
    while ((match = elementHrefRegex.exec(html))) {
      var id = match[1];
      var varName = match[2];
      var varDeclRegex = new RegExp(
        "(?:var|let|const)\\s+" +
  ...
  function parseSearchResults(html, base, defaultType) {
    return extractBlocks(html, "article")
      .map(function (block) {
  ...
  function extractDownloadLinks(html, base) {
    ...
    var links = parseAnchors(section, base)
      .filter(function (item) {
        return (
          item &&
          item.href &&
          !isIgnoredAnchorLink(item.href) &&
          !/zip/i.test(String(item.text || ""))
        );
      })
      .map(function (item) {
        return item.href;
    ...
    var buttonLinks = [];
    var buttonRegex =
      /<a\b[^>]*href=["']([^"']+)["'][^>]*>(?:(?!<\/a>)[\s\S])*?<button\b[^>]*class=["'][^"']*dwd-button[^"']*["'][^>]*>/gi;
    var buttonMatch;
    ...
    if (buttonLinks.length)
      return uniqueBy(buttonLinks, function (item) {
        return item;
      });

    return uniqueBy(
      parseAnchors(html, base)
        .filter(function (item) {
          var href = String((item && item.href) || "");
          var text = String((item && item.text) || "");
          if (!/^https?:\/\//i.test(href)) return false;
          if (baseOrigin(href) === baseOrigin(base)) return false;
          if (isIgnoredAnchorLink(href) || /hianime|fuckmaza/i.test(href))
            return false;
          if (/zip/i.test(text)) return false;
          return /download|watch|server|drive|cloud|direct|gdflix|hubcloud|filepress|filebee|pixeldrain|gofile|m4ulinks|mdrive/i.test(
            text + " " + href,
          );
        })
        .map(function (item) {
  ...
  function extractStoryline(html) {
    var regex =
      /<h3\b[^>]*class=["'][^"']*movie-title[^"']*["'][^>]*>([\s\S]*?)<\/h3>\s*<p\b[^>]*>([\s\S]*?)<\/p>/gi;
    var match;
    while ((match = regex.exec(String(html || "")))) {
  ...
  function extractDurationFromHtml(html) {
    ...
    var match = text.match(
      /(?:Duration|Runtime|Length)[:\s]*(\d+)\s*(?:min|minutes?|mins?|m\b)/i,
    );
    if (match) return Number(match[1]);
    var hMatch = text.match(
      /(\d+)\s*h(?:r|our)?s?\s*(?:\d+)?\s*(?:min|minutes?|mins?)?/i,
    );
    if (hMatch) {
  ...
  function fetchJsonWithRetry(url, headers, attemptsLeft) {
    return request(url, {
      ...
      .then(function (res) {
        ...
        if (attemptsLeft > 1)
          return fetchJsonWithRetry(url, headers, attemptsLeft - 1);
        return {};
      })
      .catch(function () {
        if (attemptsLeft > 1)
  ...
  function getTmdbFallbackPath(path) {
    ...
    if (value.indexOf(TMDB_API) === 0)
      return TMDB_FALLBACK_API + value.slice(TMDB_API.length);
    if (value.indexOf(TMDB_WORKER_API) === 0)
      return TMDB_FALLBACK_API + value.slice(TMDB_WORKER_API.length);
    return "";
  }
  ...
  function fetchTmdbJson(path) {
    ...
    tmdbInflight[key] = fetchJsonWithRetry(key, defaultHeaders(), 2)
      .then(function (json) {
  ...
  function fetchTmdbLogoUrl(type, tmdbId) {
    ...
    return fetchTmdbJson(
      ...
      for (var i = 0; i < logos.length; i++) {
        ...
        if ((logo.iso_639_1 || "").toLowerCase() === "en" && !isSvg)
          return "https://image.tmdb.org/t/p/w500" + logo.file_path;
        if ((logo.iso_639_1 || "").toLowerCase() === "en" && isSvg && !bestSvg)
          bestSvg = logo;
        if (
          !best ||
  ...
  function normalizeTmdbTitle(value) {
    return String(value || "")
      ...
      .replace(/[^a-z0-9]+/g, " ")
      .replace(
        /\b(?:season\s*\d+|episode\s*\d+|ep\s*\d+|added|web\s*dl|bluray|hindi|org|dual|multi|audio|complete|movie|download)\b/gi,
        " ",
      )
      .replace(/\s+/g, " ")
      .trim();
  ...
  function scoreTmdbResult(result, expectedTitle, expectedYear) {
    ...
    var name = normalizeTmdbTitle(
      result.name ||
        result.title ||
        result.original_name ||
        result.original_title,
    );
    var original = normalizeTmdbTitle(
    ...
    for (var i = 0; i < expectedWords.length; i++) {
      ...
      if (name.indexOf(word) !== -1 || original.indexOf(word) !== -1)
        score += 12;
    }
  ...
  function sourceLabelFromUrl(url) {
    ...
    if (/hubcloud\.|gamerxyt\.com\/hubcloud\.php/.test(value))
      return "Movies4u [HubCloud]";
    ...
    if (/multiup|validate\.multiup2\.workers\.dev/.test(value))
      return "Movies4u [MultiUp]";
    return "Movies4u";
  }
  ...
  function isDirectMediaUrl(url) {
    var val = String(url || "");
    return (
      /\.(m3u8|mp4|mkv|avi|mov|webm)(?:[?#]|$)/i.test(val) ||
      /\/api\/file\/.+/i.test(val) ||
  ...
  function looksLikeGoogleDriveUrl(url) {
    return /(?:drive|docs)\.google\.com|googleusercontent\.com/i.test(
      String(url || ""),
    );
  }
  ...
  function isPixeldrainResult(item) {
    return /pixeldrain\.(dev|com)\/api\/file\//i.test(
      String((item && item.url) || ""),
    );
  }

  function isEphemeralResolvedUrl(url) {
    return /instant\.busycdn\.xyz|hub\.diskcdn\.buzz|rest\.awscdn\.rest|video-downloads\.googleusercontent\.com|\/cdn-cgi\/content\?id=/i.test(
      String(url || ""),
    );
  }
  ...
  function isInterestingExtractorUrl(url) {
    ...
    if (
      /\/cdn-cgi\/challenge-platform\/|\/drive\/admin(?:[/?#]|$)|\/login(?:[/?#]|$)|t\.me\/|telegram|tinyurl\.com\/|winexch\.com/i.test(
        value,
      )
    )
      return false;
  ...
  function isIgnoredAnchorLink(url) {
    return /\/drive\/admin(?:[/?#]|$)|\/login(?:[/?#]|$)|t\.me\/|telegram|tinyurl\.com\/|winexch\.com|how-to-download/i.test(
      String(url || ""),
    );
  }
  ...
  function isRawExtractorCandidate(url) {
    var value = String(url || "");
    if (!/^https?:\/\//i.test(value) || isIgnoredAnchorLink(value))
      return false;
    return /m4ulinks|filesdl\.|hubcloud\.|gamerxyt\.com\/hubcloud\.php|hubdrive\.|filepress\.|filebee|gofile\.io|mdrive\.ink\/|vcloud\.zip|fastdl\.zip|multiup|validate\.multiup2\.workers\.dev|shikshakdaak/i.test(
      value,
    );
  }
  ...
  function isRelevantHubCloudAnchor(anchor) {
    ...
    if (!href || isIgnoredAnchorLink(href)) return false;
    return (
      /pixeldra|pixel\.|diskcdn|awscdn|buzzserver|fuckingfast|video-downloads\.googleusercontent\.com|instant\.busycdn\.xyz|gofile|filepress|filebee|drive\.google/i.test(
        href,
      ) ||
      /download file|pixel|pixeldrain|fsl|s3 server|mega server|buzzserver|buzz server|buzz|fuckingfast|10gbps|fast download|fslv2|gofile|filepress|filebee|drive|instant/i.test(
        text,
      )
    );
  }
  ...
  function normalizeExtractedUrl(rawValue, base) {
    ...
    if (/^(?:javascript|mailto|tel|data):/i.test(value) || /^#/.test(value))
      return "";
  ...
  function addExtractedUrl(out, seen, rawValue, base) {
    ...
    for (var index = 0; index < queue.length; index++) {
      ...
      if (
        /^[A-Za-z0-9+/_-]{24,}={0,2}$/.test(fingerprint) &&
        !/^https?:\/\//i.test(fingerprint)
      ) {
  ...
  function extractInterestingExtractorUrls(html, base) {
    ...
    while ((base64Match = quotedBase64.exec(source))) {
      ...
      if (
        ...
        (/https?:\/\//i.test(decoded) ||
          /hubcloud|filepress|filebee|drive\.google|gdfli?x|gofile|pixeldrain/i.test(
            decoded,
          ))
      ) {
    ...
    return uniqueBy(out, function (item) {
      return item;
    });
  }
  ...
  function cleanHubTitle(title) {
    ...
    return uniqueBy(result, function (item) {
      return item;
    }).join(" ");
  }
  ...
  function resolveFilesdl(url) {
    return getText(
      ...
      var qualityTitle = firstMatch(html, [
        /<div\b[^>]*class=["'][^"']*title[^"']*["'][^>]*>([\s\S]*?)<\/div>/i,
      ]);
      var quality =
        getQualityFromText(qualityTitle) || getQualityFromText(html);
      var anchors = parseAnchors(html, baseOrigin(url));
      return Promise.all(
        anchors.map(function (anchor) {
          var label = String(anchor.text || "").toLowerCase();
          if (/hubcloud/.test(label))
            return resolveExtractorUrl(anchor.href, "Filmyfiy");
          if (/gdfli?x/.test(label))
            return resolveExtractorUrl(anchor.href, "Filmyfiy");
          if (/gofile/.test(label))
            return resolveExtractorUrl(anchor.href, "Filmyfiy");
          if (/direct download|ultra fastdl|fast cloud-02/.test(label)) {
            return request(anchor.href, {
  ...
  function resolveFinalUrl(startUrl, customHeaders, maxRedirects) {
    ...
    function step(hops) {
      ...
      if (dlMatch) {
        try {
          return Promise.resolve(decodeURIComponent(dlMatch[1]));
        } catch (_) {
  ...
  function resolveBuzzserver(url, refererLabel) {
    ...
    return request(url, { headers: browserHeaders, allowRedirects: true })
      .then(function (pageRes) {
        var html = pageRes.body || "";
        var hxMatch =
          html.match(/a[^>]+hx-get=["']([^"']*download[^"']*)["']/i) ||
          html.match(/hx-get=["']([^"']+)["']/i);
        var hxGet = hxMatch ? hxMatch[1] : null;
        var copyMatch = html.match(/copyDownloadLink\(['"]([^'"]+)['"]\)/i);
        var copyLink = copyMatch
          ? copyMatch[1].replace(/\\\//g, "/")
          : null;
        var downloadUrl = hxGet
          ? absoluteUrl(baseOrigin(url), hxGet)
          : copyLink
            ? absoluteUrl(baseOrigin(url), copyLink)
        ...
        return request(downloadUrl, {
          ...
          var redirectUrl =
            (dlRes.headers &&
              (dlRes.headers["hx-redirect"] ||
                dlRes.headers["HX-Redirect"] ||
                dlRes.headers.location ||
                dlRes.headers.Location)) ||
            dlRes.finalUrl ||
            downloadUrl;
          return [
            buildStreamResult(redirectUrl, ref + " [BuzzServer]", {}, quality),
          ];
        });
      })
      .catch(function () {
  ...
  function resolveHubCloudAnchor(anchor, ref, suffix, quality, jsHrefs) {
    ...
    if (
      /video-downloads\.googleusercontent\.com|instant\.busycdn\.xyz|fastcdn-dl\.pages\.dev|rest\.awscdn\.rest|hub\.diskcdn\.buzz|cdn\.[a-z0-9.-]*buzz/i.test(
        href,
      )
    ) {
      return Promise.resolve([
    ...
    if (/fsl server/.test(label))
      return Promise.resolve([
        buildStreamResult(href, ref + " [FSL Server] " + suffix, {}, quality),
      ]);
    if (/buzzserver|buzz server|buzz|fuckingfast/.test(label) || /buzzserver|fuckingfast/i.test(href)) {
      return resolveBuzzserver(href, ref)
        .then(function (streams) {
          if (streams && streams.length) {
            return streams.map(function (s) {
              return buildStreamResult(
                s.url,
                ref + " [BuzzServer] " + suffix,
                s.headers || {},
                quality,
              );
            });
    ...
    if (/pixeldra|pixelserver|pixel server|pixeldrain/.test(label) || /pixeldra/i.test(href)) {
      var pxlLink = href;
      if (
        /negn6f/i.test(pxlLink) ||
        (!/\/u\//.test(pxlLink) && !/\/api\//.test(pxlLink))
      ) {
      ...
      return Promise.resolve([
        buildStreamResult(finalUrl, ref + " Pixeldrain " + suffix, {}, quality),
      ]);
    }
    if (/10gbps|fast download|download file/i.test(label) || /10gbps/i.test(href)) {
      return resolveFinalUrl(href, { Referer: href })
        .then(function (finalUrl) {
          var dlMatch = finalUrl.match(/dl\.php\?link=([^&]+)/i);
          if (dlMatch) {
            try {
              finalUrl = decodeURIComponent(dlMatch[1]);
            } catch (_) {
              finalUrl = dlMatch[1];
            }
          }
    ...
    if (/s3 server/.test(label))
      return Promise.resolve([
        buildStreamResult(href, ref + " [S3 Server] " + suffix, {}, quality),
      ]);
    if (/fslv2/.test(label))
      return Promise.resolve([
        buildStreamResult(href, ref + " [FSLv2] " + suffix, {}, quality),
      ]);
    if (/mega server/.test(label))
      return Promise.resolve([
        buildStreamResult(href, ref + " [Mega Server] " + suffix, {}, quality),
      ]);
  ...
  function resolveHubCloud(url, refererLabel) {
    ...
    return hrefPromise.then(function (innerUrl) {
      return getText(innerUrl, refHeaders).then(function (html) {
        ...
        var anchors = parseAnchors(html, baseOrigin(innerUrl)).filter(
      ...
      if (/gofile\.io/i.test(String(url || ""))) {
        return Promise.resolve([
          buildStreamResult(
            url,
            "Gofile",
            defaultHeaders({ Referer: "https://gofile.io/" }),
    ...
    function resolveFilepressContent(contentUrl) {
      ...
      return request(contentUrl, {
        ...
        .then(function (initialRes) {
          var redirectLocation = normalizeExtractedUrl(
            initialRes.headers.location ||
              initialRes.headers["x-redirect-location"] ||
              "",
            baseOrigin(contentUrl),
          );
          if (redirectLocation && redirectLocation !== contentUrl) {
            if (looksLikeGoogleDriveUrl(redirectLocation))
              return resolveGoogleDrive(redirectLocation);
            if (isDirectMediaUrl(redirectLocation)) {
              return [
                buildStreamResult(
                  ...
                  "Filepress",
                  {},
                  getQualityFromText(redirectLocation),
                ),
              ];
            }
            ...
            .then(function (pageRes) {
              var finalUrl =
                normalizeExtractedUrl(
                  pageRes.finalUrl || contentUrl,
                  baseOrigin(contentUrl),
                ) || contentUrl;
              if (looksLikeGoogleDriveUrl(finalUrl))
                return resolveGoogleDrive(finalUrl);
              ...
              if (
                !candidates.length &&
                finalUrl !== contentUrl &&
                isInterestingExtractorUrl(finalUrl)
              ) {
              ...
              return Promise.all(
                candidates.map(function (candidate) {
                  if (looksLikeGoogleDriveUrl(candidate))
                    return resolveGoogleDrive(candidate);
    ...
    return request(url, {
      ...
      .then(function (initialRes) {
        ...
        return request(url, {
          ...
          .then(function (pageRes) {
            var finalUrl =
              normalizeExtractedUrl(pageRes.finalUrl || url, baseOrigin(url)) ||
              url;
            if (looksLikeGoogleDriveUrl(finalUrl))
              return resolveGoogleDrive(finalUrl);
            if (isDirectMediaUrl(finalUrl)) {
            ...
            var hiddenContentLinks = parseAnchors(html, pageBase)
              .map(function (anchor) {
      ...
      bases.map(function (base) {
        ...
        return request(indexbotLink, {
          ...
          .then(function (res) {
            ...
            var phpsessidMatch =
              String(cookieHeader).match(/PHPSESSID=([^;]+)/i);
            var phpsessid = phpsessidMatch ? phpsessidMatch[1] : "";
            var token = firstMatch(html, [
              /formData\.append\('token', '([a-f0-9]+)'\)/i,
            ]);
            var postId = firstMatch(html, [
              /fetch\('\/download\?id=([a-zA-Z0-9/+]+)'/i,
            ]);
            if (!token || !postId) return [];
            return postForm(
              base + "/download?id=" + postId,
              { token: token },
              {
              ...
              var downloadLink = firstMatch(postRes.body, [
                /url":"(.*?)"/i,
              ]).replace(/\\/g, "");
  ...
  function resolveMultiupMirror(url, fileName, fileSize, quality) {
    return getText(url, defaultHeaders({ Referer: baseOrigin(url) + "/" }))
      .then(function (html) {
        var anchors = parseAnchors(html, baseOrigin(url)).filter(
          function (anchor) {
        ...
        return Promise.all(
          anchors.map(function (anchor) {
            ...
            var label = String(anchor.text || "").toLowerCase();
            if (/gofile\.io/i.test(href) || /gofile/i.test(label))
              return resolveGofile(href);
            if (/megaup\.net/i.test(href))
              return Promise.resolve([
                buildStreamResult(
                  href,
                  "GDFlix[Mirror Megaup] " + fileName + "[" + fileSize + "]",
                  {},
                  quality,
                ),
              ]);
            if (/1fichier\.com/i.test(href))
              return Promise.resolve([
                buildStreamResult(
                  href,
                  "GDFlix[Mirror 1fichier] " + fileName + "[" + fileSize + "]",
                  {},
                  quality,
                ),
              ]);
            if (/download-fast\//i.test(href))
              return Promise.resolve([
                buildStreamResult(
                  absoluteUrl(baseOrigin(url), href),
                  "GDFlix[Mirror Direct] " + fileName + "[" + fileSize + "]",
                  {},
                  quality,
                ),
              ]);
  ...
  function resolveHubDrive(url) {
    var headers = defaultHeaders({ Referer: baseOrigin(url) + "/" });
    return getText(url, headers, true)
      .then(function (html) {
        var candidates = extractInterestingExtractorUrls(
          html,
          baseOrigin(url),
    ...
    return getText(url, headers)
      .then(function (html) {
        var candidates = extractInterestingExtractorUrls(
          ...
          return (
            candidate !== url &&
            /hubcloud\.|gdflix\.|filepress\.|g-direct\.|drive\.google|filebee/i.test(
              candidate,
            )
          );
        });
            ...
            return Promise.all(
              ...
              .then(function (results) {
                ...
                return candidates
                  .filter(function (candidate) {
                    return (
                      looksLikeGoogleDriveUrl(candidate) ||
                      /hubcloud\.|hubdrive\.|filepress\.|filebee/i.test(
                        candidate,
                      )
                    );
                  })
                  .map(function (candidate) {
                    var label = looksLikeGoogleDriveUrl(candidate)
                      ...
                      : /hubdrive\./i.test(candidate)
                        ? "HubDrive"
                        : /filepress\.|filebee/i.test(candidate)
                          ? "Filepress [GDrive]"
                          : "HubCloud";
                    return buildStreamResult(
                      candidate,
  ...
  function withTimeout(promise, ms, label) {
    ...
    var timeoutPromise = new Promise(function (_, reject) {
      timer = setTimeout(function () {
        reject(
          new Error("Timeout after " + ms + "ms for " + (label || "request")),
        );
      }, ms);
    });
  ...
  function resolveExtractorUrl(url, refererLabel) {
    ...
    if (/hubdrive\./i.test(url))
      return withTimeout(resolveHubDrive(url), 20000, "HubDrive");
    ...
    try {
      ...
      if (trendingItems.length) {
        ...
        for (var r = 0; r < searchResponses.length; r++) {
          ...
          var itemYearMatch = String(trendingItems[r].title || "").match(
            /\((\d{4})\)/,
          );
          var itemYear = itemYearMatch ? itemYearMatch[1] : "";
          var bestResult = chooseBestTmdbResult(
            searchJson.results,
            trendingItems[r].title,
            itemYear,
          );
        ...
        var detailResponses = detailReqs.length
          ? await httpParallelGet(detailReqs)
          : [];
        for (var d = 0; d < detailResponses.length; d++) {
          var origItemIdx = detailOrigIdx[d];
          var details = parseJsonSafe(detailResponses[d].body, {});
          var imdbId =
            details && details.external_ids && details.external_ids.imdb_id
          ...
          var logoUrl = imdbId
            ...
            : "";
          var bannerPath =
            details && details.backdrop_path ? details.backdrop_path : "";
      ...
      if (!total) {
        cb({
          ...
          message: "No home items parsed. Snippet: " + snippet(firstHtml, 320),
        });
        return;
      }

      Analytics.logEvent("movies4u_home", {});
  ...
  async function load(url, cb) {
    try {
      ...
      var poster =
        ...
        getImageFromBlock(
          firstMatch(html, [
            /<div\b[^>]*class=["'][^"']*post-thumbnail[^"']*["'][^>]*>([\s\S]*?)<\/div>/i,
          ]),
          sourceUrl,
        );
      ...
      if (title === "Unknown Title" && !downloadLinks.length && !seasonSections.length) {
        cb({
          ...
          message: "Load page not parsed. Snippet: " + snippet(html, 320),
        });
        return;
      }

      var tmdbId = await findTmdbId(
        imdbId,
        isMovie ? "movie" : "tv",
        title,
        titleYear,
      ...
      var detailsPromise = tmdbId
        ? fetchTmdbDetails(isMovie ? "movie" : "tv", tmdbId)
        : Promise.resolve({});
        ...
        var seasonResults = await Promise.all(
          seasonSections.map(async function (section) {
            var seasonMetadata = tmdbId
              ? await fetchSeasonMetadata(tmdbId, section.season)
              : {};
            var seasonEpisodeNumbers = getSeasonEpisodeNumbers(
              seasonMetadata,
              section.season,
            );
            var resolvedBlocks = await Promise.all(
              (section.links || []).map(function (qualityLink) {
                return getText(
                  qualityLink,
                  defaultHeaders({ Referer: sourceUrl }),
                )
          ...
          for (var j = 0; j < seasonResult.resolvedBlocks.length; j++) {
            ...
            if (resolvedBlock.episodeBlocks.length) {
              ...
              var fallbackEpisodes = seasonResult.seasonEpisodeNumbers.length
                ? seasonResult.seasonEpisodeNumbers
                : [1];
              for (
                var epIndex = 0;
                epIndex < fallbackEpisodes.length;
                epIndex++
              ) {
      ...
      var logoUrl =
        tmdbDetails &&
        tmdbDetails.external_ids &&
        tmdbDetails.external_ids.imdb_id
          ? "https://live.metahub.space/logo/medium/" +
            tmdbDetails.external_ids.imdb_id +
            "/img"
          : "";
      if (tmdbDetails) {
      ...
      var bannerUrl =
        tmdbDetails && tmdbDetails.backdrop_path
          ? TMDB_IMAGE_BASE + tmdbDetails.backdrop_path
          : poster;
      var year = trim(
        (
          (tmdbDetails &&
      ...
      var cast = buildActorList(
        tmdbDetails && tmdbDetails.credits && tmdbDetails.credits.cast,
      );
      var score =
        tmdbDetails && typeof tmdbDetails.vote_average === "number"
          ? Number(tmdbDetails.vote_average.toFixed(1))
          : undefined;
      var trailerUrl =
        extractTmdbTrailerUrl(tmdbDetails) ||
        extractTrailerFromHtml(html, sourceUrl);
      var duration =
        extractTmdbDuration(tmdbDetails, isMovie ? "movie" : "tv") ||
        extractDurationFromHtml(html);
      var genres = extractTmdbGenres(tmdbDetails);

      if (isMovie) {
        var movieLinks = uniqueBy(downloadLinks, function (item) {
          return item;
        });
        cb({
          success: true,
      ...
      var episodes = Object.keys(episodeLinksMap)
        .sort(function (a, b) {
        ...
        .map(function (key) {
          ...
          return new Episode({
            ...
            score: meta.rating
              ? Number(Number(meta.rating).toFixed(1))
              : undefined,
            headers: defaultHeaders({ Referer: sourceUrl }),
          });
        });

      Analytics.logEvent("movies4u_load", {});
      cb({
        success: true,
        data: new MultimediaItem({
          title: title,
  ...
  async function loadStreams(url, cb) {
    try {
      ...
      var resolved = await Promise.all(
        (payload.links || []).map(function (link) {
          ...
          var rawFallback = [
            buildStreamResult(
              link,
              sourceLabelFromUrl(link),
              defaultHeaders({
                Referer: payload.sourceUrl || baseOrigin(link) + "/",
              }),
              quality,
            ),
          ];
      ...
      // Keep all direct stream urls (Pixeldrain, FSL, etc.) without discarding Pixeldrain results when other sources are available.

      Analytics.logEvent("movies4u_loadstreams", {});
      cb({
        success: true,
