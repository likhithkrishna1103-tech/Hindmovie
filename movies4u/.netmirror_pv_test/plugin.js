(function () {
    ...
    function resolveProviderKey() {
        var candidates = [
            (typeof manifest !== "undefined" && manifest && manifest.providerId) ? manifest.providerId : "",
            (typeof manifest !== "undefined" && manifest && manifest.name) ? manifest.name : ""
        ];
        for (var i = 0; i < candidates.length; i++) {
            var normalized = normalizeProviderToken(candidates[i]);
            for (var key in PROVIDER_CONFIGS) {
                if (!Object.prototype.hasOwnProperty.call(PROVIDER_CONFIGS, key)) continue;
                if (normalizeProviderToken(key) === normalized) return key;
                if (normalizeProviderToken(PROVIDER_CONFIGS[key].displayName) === normalized) return key;
            }
        }
        return DEFAULT_PROVIDER_KEY;
    }

    var CONFIG = PROVIDER_CONFIGS[resolveProviderKey()] || PROVIDER_CONFIGS[DEFAULT_PROVIDER_KEY];
    ...
    function getSelectedBaseUrl() {
        return normalizeBaseUrl(CONFIG.defaultBaseUrl) || CONFIG.defaultBaseUrl;
    }
    ...
    function getCookieBases() {
        return unique((CONFIG.cookieBases || []).concat(getStreamBases()));
    }
