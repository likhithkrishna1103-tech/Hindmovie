(function() {
    ...
    const PROVIDER_PACKAGE_MAP = {
        jio: "com.igris.repo.playztvjio",
        zee: "com.igris.repo.playztvzee",
        sony: "com.igris.repo.playztvsony",
        bdixtv: "com.igris.repo.playztvbdixtv",
        sunnxt: "com.igris.repo.playztvsunnxt",
        dishtv: "com.igris.repo.playztvdishtv",
        freesports: "com.igris.repo.playztvfreesports",
    ...
    async function decryptBundledPlaylistContent(content) {
        ...
        if (!text || isPlaylistResponse(text)) {
            return text;
        }
        ...
        try {
            ...
            const ctBytes = base64ToBytes(encryptedData);
            const plain = AES.decryptCbc(ctBytes, keyBytes, ivBytes);
            const decrypted = bytesToUtf8(plain);
    ...
    async function getHome(cb) {
        try {
            ...
            if (specialProvider) {
                const specialResult = await resolveSpecialProviderChannels(specialProvider);
                if (specialResult.channels.length) {
                    const sections = buildSectionsFromChannels(specialResult.channels);
                    Analytics.logEvent('playztv_home', {});
                ...
                return cb({
                    ...
                    message: specialResult.error || `No channels found for ${specialProvider.label}`
                });
            }
            ...
            if (!config) {
                return cb({
                    ...
                    message: "Unsupported PlayZTV provider requested"
                });
            }
            ...
            providerResults.forEach((result) => {
                if (result.channels.length) {
                    ...
                    failures.push(`${result.label}: ${result.error}`);
                }
            });

            // Also load custom categories (FIFA WC26 Highlights etc.)
            ...
            customResults.forEach(function(result) {
                if (result.channels.length) {
                    result.channels.forEach(function(channel) { channels.push(channel); });
                }
            });
            ...
            if (!channels.length) {
                return cb({
                    ...
                    message: failures.length ? failures.join(" | ") : "No channels resolved from the configured PlayZTV sources"
                });
            }
            ...
            channels.forEach((channel) => {
                ...
                sections[groupName].push(createChannelItem(channel, !!config.merged, channel.__customProvider ? "movie" : "livestream"));
            });
    ...
    async function load(urlStr, cb) {
        try {
            ...
            Analytics.logEvent('playztv_load', {});
            cb({
                success: true,
